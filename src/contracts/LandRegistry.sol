...
contract LandRegistry {
    ...
    function govtStatus(uint256 _id,string memory status,string memory _isAvailable) public returns (bool) {
    ...
    function processRequest(uint256 property, reqStatus status) public {
        require(land[property].id == msg.sender);
        land[property].requestStatus = status; //Approve or rejected
    ...
    function buyProperty(uint256 property) public payable {
        ...
        land[property].requester = address(0);
        land[property].requestStatus = reqStatus.Default;
        profile[msg.sender].assetList.push(property); //adding new property to the user
    }

    function removeOwnership(address previousOwner, uint256 id) private {
        uint256 index = findId(id, previousOwner);
        profile[previousOwner].assetList[index] = profile[previousOwner].assetList[profile[previousOwner].assetList.length - 1]; //copying last asset
        delete profile[previousOwner].assetList[profile[previousOwner].assetList.length - 1]; //deleting last
        profile[previousOwner].assetList.length--; //decreasing length
    }
